...
var cfg = {
  ...
  colors: ["b5e9e9", "fef6dd", "ffe1d0", "fff1b5", "dcf3d0"]
  /*colors: [
    "FFAACC",
    "FFBBCC",
    "FFCCCC",
...
function upColor() {
  var colorInterval =
    cfg.colorMinInterval +
    Math.floor(Math.random() * Math.floor(cfg.colorMaxInterval));
  var newIdx = cfg.colorIdx + colorInterval;
  if (newIdx >= cfg.colors.length) {
    newIdx = newIdx % cfg.colors.length;
  }
  cfg.colorIdx = newIdx;
}
*/
...
function render(node, depth) {
  ...
  $("#left" + depth).one(
    "click",
    function(node, depth) {
      //console.log("You clicked", node.left);
      if (node.left.left) {
